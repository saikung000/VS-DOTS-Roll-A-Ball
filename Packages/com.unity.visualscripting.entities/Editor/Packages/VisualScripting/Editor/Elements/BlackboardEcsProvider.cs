...
namespace Packages.VisualScripting.Editor.Elements
{
    public class BlackboardEcsProvider : IBlackboardProvider
    {
        ...
        public void DisplayAppropriateSearcher(Vector2 mousePosition, VseGraphView graphView)
        {
            ...
            if (picked != null)
            {
                Blackboard blackboard = picked.GetFirstAncestorOfType<Blackboard>();
                ...
                else if (picked is ComponentQuery group)
                {
                    var componentsSubSection = picked.GetFirstAncestorOfType<ComponentsSubSection>();
                    if (componentsSubSection != null)
                    {
                        componentsSubSection.AddComponentToQuery(group.ComponentQueryDeclarationModel);
                    }
                    else
                    {
                        var criteriaSubSection = picked.GetFirstAncestorOfType<CriteriaSubSection>();
                        criteriaSubSection?.AddCriteriaModel(group.ComponentQueryDeclarationModel);
                    }
                }
