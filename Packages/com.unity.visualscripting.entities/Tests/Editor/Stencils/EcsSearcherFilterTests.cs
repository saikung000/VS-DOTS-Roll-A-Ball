...
namespace UnityEditor.VisualScriptingECSTests.Stencils
{
    class EcsSearcherFilterTests
    {
        ...
        {
            var filter = new EcsSearcherFilter(SearcherContext.Type).WithComponentData(m_Stencil);
        ...
        public void TestWithComponentDataWithExclusion()
        {
            ...
            var filter = new EcsSearcherFilter(SearcherContext.Type).WithComponentData(m_Stencil, new HashSet<TypeHandle>(){componentDataTypeHandle});
