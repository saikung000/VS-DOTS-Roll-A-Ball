...
namespace UnityEditor.VisualScripting.GraphViewModel
{
    ...
    public abstract class NodeModel : INodeModel, ISerializationCallbackReceiver
    {
        ...
        [SerializeField]
        GroupNodeAsset m_GroupNodeModel;
        [SerializeField]
        Vector2 m_Position;
        ...
        {
            get => m_GroupNodeModel == null? null : m_GroupNodeModel.Node;
            set => m_GroupNodeModel = value?.NodeAssetReference as GroupNodeAsset;
        }
        ...
        static PortModel ReuseOrCreatePortModel(PortModel model, IReadOnlyDictionary<string, IPortModel> previousPorts, OrderedPorts newPorts)
        {
            ...
            if (previousPorts.TryGetValue(id, out var existingModel))
            {
                ...
                modelToAdd.Name = existingModel.Name;
                modelToAdd.DataType = existingModel.DataType;
                modelToAdd.PortType = existingModel.PortType;
            }
        ...
        {
            Undo.RecordObject(NodeAssetReference, "Move");
            Position = position;
        }

        public void ChangeColor(Color color)
        {
            Undo.RecordObject(NodeAssetReference, "Change Color");
            HasUserColor = true;
            m_Color = color;
        }
        ...
        {
            if (m_GroupNodeModel)
                m_GroupNodeModel.Node.RemoveNode(this);
            if(m_AssetReference != null)
                Undo.DestroyObjectImmediate(m_AssetReference);
        }
