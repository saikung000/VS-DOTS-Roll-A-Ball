...
namespace UnityEditor.VisualScripting.Editor.SmartSearch
{
    public static class SearcherService
    {
        ...
        internal static void ShowGroupNodes(State state, Vector2 position, Action<GraphNodeModelSearcherItem> callback)
        {
            var stencil = state.CurrentGraphModel.Stencil;
            var filter = stencil.GetSearcherFilterProvider()?.GetGroupSearcherFilter();
            var adapter = new GraphNodeSearcherAdapter(state.CurrentGraphModel, "Add a Node to the group");
            var dbProvider = stencil.GetSearcherDatabaseProvider();
            var dbs = dbProvider.GetGraphElementsSearcherDatabases()
