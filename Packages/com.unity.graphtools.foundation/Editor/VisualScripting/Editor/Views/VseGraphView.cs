    ...
    public class VseGraphView : GraphView, IDropTarget
    {
        ...
        public static string OnSerializeGraphElements(IEnumerable<GraphElement> elements)
        {
            var copyPasteData = GatherCopiedElementsData(o =>
            {
                ...
                var copiedLength = clone.name.Length - k_CloneStringLength;
                if(copiedLength > 0)
                    clone.name = clone.name.Substring(0, copiedLength);
        ...
        internal static CopyPasteData GatherCopiedElementsData(Func<ScriptableObject, ScriptableObject> map, IReadOnlyCollection<IGraphElementModel> graphElementModels)
        {
            ...
            List<ScriptableObject> nodesToCopy = originalNodes
                .Select(model => model.NodeAssetReference)
                .Select(map)
                .ToList();
            ...
            foreach (var asset in nodesToCopy.OfType<AbstractNodeAsset>())
            {
                ...
                if (asset is IStackAsset stackAsset)
                {
                    ...
                    {
                        var stackedNodeAssetToCopy = (AbstractNodeAsset)map(stackedNodeModel.NodeAssetReference);
...
//        }
        ...
        {
            // Find the NodeAsset associated with the semantic node's instance id
            var nodeAsset = EditorUtility.InstanceIDToObject(nodeInstanceId) as AbstractNodeAsset;

            GraphElement graphElement = this.Query<Node>().Where(e => ReferenceEquals(e.model.NodeAssetReference, nodeAsset)).First();
            if (graphElement == null)
            {
                var methodUINodes = this.Query<FunctionNode>().ToList();
                graphElement = methodUINodes?.FirstOrDefault(e => e.m_FunctionModel.NodeAssetReference == nodeAsset);
            }

            if (graphElement == null)
            {
                var stackUINodes = this.Query<StackNode>().ToList();
                graphElement = stackUINodes?.FirstOrDefault(e => ReferenceEquals(e.GraphElementModel, nodeAsset.Model));
            }

            if (graphElement == null)
                graphElement = this.Query<Token>().Where(e => ReferenceEquals(e.GraphElementModel, nodeAsset.Model)).First();
