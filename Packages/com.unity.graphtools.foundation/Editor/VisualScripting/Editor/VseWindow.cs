        ...
        internal void DisplaySmartSearch(DropdownMenuAction menuAction = null, int insertIndex = -1)
        {
            ...
            switch (lastHoveredElement)
            {
                ...
                case Group groupNode:
                    SearcherService.ShowGroupNodes(m_Store.GetState(), mousePosition, item =>
                    {
                        m_Store.Dispatch(new CreateGroupedNodeFromSearcherAction(
                            m_Store.GetState().CurrentGraphModel, groupNode.model, graphPosition, item));
                    });
                    break;
