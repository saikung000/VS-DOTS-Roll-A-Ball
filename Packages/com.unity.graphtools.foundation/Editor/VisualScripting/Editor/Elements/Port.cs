    ...
    public class Port : Experimental.GraphView.Port, IDropTarget
    {
            ...
            IEnumerable<IEdgeModel> edgesToDelete, IStackModel stackModel, int index, IGroupNodeModel groupModel)
        {
            switch (portModel.PortType)
            {
                ...
                case PortType.Loop:
                    store.Dispatch(new CreateNodeFromLoopPortAction(portModel, position, edgesToDelete, groupModel));
                    break;
            }
        }
        ...
        public bool DragEnter(DragEnterEvent evt, IEnumerable<ISelectable> selection, IDropTarget enteredTarget, ISelection dragSource)
        {
            ...
            EnableInClassList(k_DropHighlightDeniedClass, !CanAcceptDrop(selection.ToList()));
            return true;
        }
