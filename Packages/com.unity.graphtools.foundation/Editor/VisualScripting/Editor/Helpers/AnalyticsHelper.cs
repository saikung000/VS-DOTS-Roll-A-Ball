...
namespace UnityEditor.VisualScripting.Editor
{
    ...
    class AnalyticsHelper
    {
        class PendingNodeCreationEvent
        {
            ...
            public PendingNodeCreationEvent(int instanceId, string nodeTitle)
            {
        ...
        public void FlushNodeCreationEvent(NodeCreationEvent e, IEnumerable<int> deletedNodeIds = null)
        {
            if (m_LastNodeCreation?.Action.HasValue == true)
            {
                ...
                if (e == NodeCreationEvent.UndoOrDelete && (deletedNodeIds == null || deletedNodeIds.Contains(m_LastNodeCreation.InstanceId)))
                    vsUserActions.SetResult(NodeCreationEvent.UndoOrDelete);

                m_UserActionEvents.Enqueue(vsUserActions);
                Log($"Flush last node creation {m_LastNodeCreation.InstanceId} result: {vsUserActions.vsSearcherCancelled}");

                m_LastNodeCreation = null;
            }
        }

        public void SetLastNodeCreated(int nodeId, string nodeTitle)
        {
