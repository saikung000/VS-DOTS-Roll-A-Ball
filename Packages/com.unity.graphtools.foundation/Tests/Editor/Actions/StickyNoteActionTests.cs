...
namespace UnityEditor.VisualScriptingTests.Actions
{
    ...
    class StickyNoteActionTests : BaseFixture
    {
        ...
        {
            var stickyNote = GraphModel.CreateStickyNote("stickyNote", k_StickyNoteRect);
        ...
        public void Test_UpdateStickyNoteThemeAction([Values] TestingMode mode)
        {
            ...
            TestPrereqActionPostreq(mode,
                () =>
                {
                    ...
                    Assert.That(GetEdgeCount(), Is.EqualTo(0));
                    Assert.That(GetStickyNoteCount(), Is.EqualTo(1));
                    Assert.That(GetStickyNote(0).Theme, Is.EqualTo(StickyNoteTheme.Classic));
                    return new UpdateStickyNoteThemeAction(new List<IStickyNoteModel> { stickyNote }, StickyNoteTheme.Teal);
                },
                () =>
                {
                    Assert.That(GetNodeCount(), Is.EqualTo(0));
                    Assert.That(GetEdgeCount(), Is.EqualTo(0));
                    Assert.That(GetStickyNoteCount(), Is.EqualTo(1));
                    Assert.That(GetStickyNote(0).Theme, Is.EqualTo(StickyNoteTheme.Teal));
                });
        }
